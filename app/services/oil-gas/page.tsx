        ...
        <ServiceLayout>
            <div className="space-y-16">
                ...
                <section className="text-center space-y-6">
                    ...
                    <div className="h-1 w-24 bg-amber-900 mx-auto rounded-full"></div>
                </section>

                {/* Introduction & Approach */}
                <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                        <h3 className="text-2xl font-bold text-amber-950 mb-4">Strategic Energy Solutions</h3>
                        <p className="text-lg text-slate-700 leading-relaxed">
                            EXIM Corporation supports energy requirements for industrial, commercial, and strategic infrastructure projects through structured partner networks, product supply, and regional partnerships. We engage selectively in petroleum product trading and energy logistics — designed to ensure reliable access to refined fuels, industrial feedstocks, and energy solutions tailored to complex project needs.
                        </p>
                    </div>
                    <div className="bg-amber-50 p-8 rounded-2xl border border-amber-100 hover:shadow-md transition-all">
                        <h3 className="text-2xl font-bold text-amber-950 mb-4">Our Approach</h3>
                        <p className="text-lg text-amber-900/80 leading-relaxed">
                            We combine global energy market insights with compliant partner strategies to deliver refined petroleum products and industrial fuels that meet international quality standards. Our operations emphasize risk aware supply chains, regulatory compliance, and long term sustainability for partners across emerging and established markets.
                        </p>
                    </div>
                </section>
                ...
                <section className="space-y-8">
                    <div className="text-center max-w-3xl mx-auto">
                        <h2 className="text-3xl font-bold text-amber-950 mb-4">Fuel & Energy Product Portfolio</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[
                            ...
                            <div key={i} className="flex items-center gap-4 p-6 bg-white rounded-xl shadow-md border-l-4 border-amber-900 hover:shadow-lg transition-all group">
                                <span className="text-2xl group-hover:scale-110 transition-transform duration-300">🛢️</span>
                                <span className="text-amber-900 font-bold">{item}</span>
                            </div>
                        ))}
                    </div>
                </section>
                ...
                {/* Services */}
                <section className="bg-amber-950 text-amber-50 rounded-3xl p-8 md:p-12 shadow-xl relative overflow-hidden">
                    {/* Texture Overlay */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {[
                                ...
                                <div key={i} className="bg-white/10 p-6 rounded-xl border border-white/10 hover:bg-white/20 transition-colors backdrop-blur-sm">
                                    <h3 className="text-xl font-bold text-amber-100 mb-3">{service.title}</h3>
                                    <p className="text-amber-100/70 leading-relaxed">{service.desc}</p>
                                </div>
                            ))}
                        </div>
                    </div>
